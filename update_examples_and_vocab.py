...
global_vocab_additions = {
    "<ruby>選びました<rt>えらびました</rt></ruby>": "To choose (past polite)",
    "みつかりました": "To be found (past polite)",
    "<ruby>弾けます<rt>ひけます</rt></ruby>": "Can play",
    "<ruby>話せませんでした<rt>はなせませんでした</rt></ruby>": "Could not speak",
    "<ruby>食べられます<rt>たべられます</rt></ruby>": "Can eat",
    "<ruby>書けば<rt>かけば</rt></ruby>": "If you write",
    "<ruby>食べてしまいました<rt>たべてしまいました</rt></ruby>": "Ended up eating entirely",
    "<ruby>引っ越す<rt>ひっこす</rt></ruby>": "To move",
    "手伝って": "To help (te-form)",
    "<ruby>運んで<rt>はこんで</rt></ruby>": "To carry (te-form)",
    "こわれました": "Broke (intransitive)",
    "おちて": "Dropped/fell (te-form)",
    "われて": "Broke/cracked (te-form)",
    "<ruby>降りそうです<rt>ふりそうです</rt></ruby>": "Looks like it will rain",
    "<ruby>結婚する<rt>けっこんする</rt></ruby>": "To marry",
    "たべられました": "Was eaten (passive)",
    "考えさせて": "Let me think (causative te-form)",
    "れんしゅうさせられました": "Was forced to practice (causative-passive)",
    "やめさせられそうに": "Almost forced to quit",
    "おもちいたします": "I will carry (humble)",
    "おうかがいしても": "May I ask (humble)",
    "おせわに": "Being taken care of",
    "<ruby>提出して<rt>ていしゅつして</rt></ruby>": "To submit (te-form)",
    "かしこまりました": "Understood (highly formal)",
    "<ruby>提出いたします<rt>ていしゅついたします</rt></ruby>": "Will submit (humble)",
    "<ruby>残業<rt>ざんぎょう</rt></ruby>": "Overtime work",
    "<ruby>承って<rt>うけたまわって</rt></ruby>": "Received/heard (humble te-form)",
    "<ruby>取りかからせられて<rt>とりかからせられて</rt></ruby>": "Was forced to start (causative passive te-form)",
    "<ruby>考えます<rt>かんがえます</rt></ruby>": "I think (polite)",
    "<ruby>自己評価<rt>じこひょうか</rt></ruby>": "Self-esteem",
    "<ruby>規制<rt>きせい</rt></ruby>": "Regulation"
}

# 1. Update examples in lessonData
def replace_examples(m):
    step_id = m.group(1)
    if step_id in updates:
        # construct the examples JSON array correctly
        # The existing structure is: "examples": [{"jp": "...", "romaji": "", "en": ""}, ...]
        exs = []
        for new_jp in updates[step_id]:
            exs.append({
                "jp": new_jp,
                "romaji": "",
                "en": ""
...
# The regex needs to match the pracStep dictionary key and its properties up to "examples": [...]
new_text = re.sub(r'(pracStep\d{2}): (\{.*?\})"examples": \[(.*?)\]', replace_examples, text, flags=re.DOTALL)

# 2. Append new vocab to GLOBAL_VOCAB
vocab_js_lines = []
for k, v in global_vocab_additions.items():
    vocab_js_lines.append(f'            "{k}": "{v}",')
vocab_add_str = "\n".join(vocab_js_lines)

# Inject into GLOBAL_VOCAB
new_text = re.sub(r'(const GLOBAL_VOCAB = \{)', r'\1\n' + vocab_add_str, new_text)
